...
- T <=Int TokenPool <=Int D by // TODO:
- Transfers(Op ;; Ops) >=Int 0 by // TODO:
...
proof sketch [only-dexter]:
- DEXTER is a smart contract
- no others can send a transaction with the sender of DEXTER
```

NOTE:
- a top-level transaction (i.e., the sender is equal to the source) cannot be generated by dexter
...
NOTE:
- the dfs execution tree model guarantees that:
  - if the current operation is top-level, then all the continuations are top-level with the same sender which is the source
